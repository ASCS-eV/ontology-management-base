...
generate:
	...
	for domain in $$DOMAINS_TO_BUILD; do \
		...
		"$(GEN_OWL)" --ontology-uri-suffix "" linkml/$$domain/$$domain.yaml > artifacts/$$domain/$$domain.owl.ttl; \
		"$(GEN_SHACL)" linkml/$$domain/$$domain.yaml > artifacts/$$domain/$$domain.shacl.ttl; \
		"$(GEN_JSONLD_CONTEXT)" linkml/$$domain/$$domain.yaml > artifacts/$$domain/$$domain.context.jsonld; \
	done
