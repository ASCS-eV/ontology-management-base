...
| **SHACL shapes** | 6 `sh:NodeShape`, 108 `sh:property`, 18 `sh:sparql` | 7 `sh:NodeShape`, 113 `sh:property`, 18 `sh:sparql` |
...
| **Schema rules** | — | 18 conditional constraints (LinkML rules) |
| **Test data** | 1 valid + 3 invalid instances | 1 valid + 11 invalid instances |
...
or false, the corresponding value must not be present"*. All 18 flag↔value
pairs (17 original + 1 new `trafficAgentTypeValue`) are now modeled as
LinkML `rules:` with `value_presence` / `equals_string` postconditions on
the `Odd` and `Behaviour` classes. A further v1 SPARQL rule enforces
`minValue ≤ maxValue` on `QuantitativeValue`.

**Schema status:** ✅ 18 rules defined in `openlabel-v2.yaml` (flag↔value).
...
**Coverage:** 18/18 boolean-guard constraints generated (100% of Pattern A).
This includes all 17 original v1 guards plus 1 new guard for
`trafficAgentTypeValue` (BSI PAS-1883 §5.4.a.4). The Pattern B cross-field
comparison (1 constraint: `minValue ≤ maxValue`) is not generated because it
is not modeled as a LinkML rule; it is handled via slot-level `minimum_value`
constraints instead.

**Impact:** v2 SHACL now enforces all conditional constraints at parity with
v1. Data with a value but no corresponding flag=true will fail validation.
...
| Invalid instances | 3 (wrong value, decimal for integer, string for boolean) | 11 (wrong value, decimal for integer, string for boolean, wrong literal types, wrong enum, array enum, cross-contamination, maxCount violation, missing required, closed shape, wrong traffic agent enum) |
| Enum value format | Prefixed IRIs (`openlabel:RoadTypeMotorway`) | Bare strings (`RoadTypeMotorway`) via `@type: @vocab` |
| `.expected` files | ✅ All 3 present | ✅ All 11 present |
...
| L1: SHACL conditional constraints | ✅ 18 rules generated via `gen-shacl --emit-rules` (ASCS-eV/linkml fork) |
...
|----------|------|--------|
| 🟡 Low | `minValue ≤ maxValue` cross-field SPARQL | Not expressible as LinkML rule; handled via slot-level `minimum_value` (JSON Schema path only) |
