...
from omb.utils import context_generator
from omb.utils.context_generator import (
    EXTERNAL_LINKML_DOMAINS,
...
class TestExternalLinkMLDomainExclusion:
    """gx is generated by an external LinkML pipeline and must never be
    overwritten by this fallback generator.

    Regression guard: the exclusion used to be inferred by string-matching
    "LinkML" in the context file's ``comments.description``. Passing
    ``--no-metadata`` to the upstream generator removed that block and silently
    re-enabled generation, which would replace the full context with a stub.
    """

    def test_gx_is_in_external_linkml_domains(self):
        assert "gx" in EXTERNAL_LINKML_DOMAINS

    def test_generate_all_contexts_never_generates_external_domains(self, monkeypatch):
        ...
        for exclude in (None, [], ["openlabel-v2"], ["something-else"]):
            called.clear()
            results = generate_all_contexts(exclude=exclude, dry_run=True)
            assert "gx" not in called, f"gx generated with exclude={exclude!r}"
            assert results.get("gx") is None

    def test_write_context_refuses_external_domain(self, monkeypatch):
        """The write path is guarded too, so --domain gx cannot clobber."""
        wrote: list[object] = []
        monkeypatch.setattr(
            context_generator,
            "write_if_changed",
