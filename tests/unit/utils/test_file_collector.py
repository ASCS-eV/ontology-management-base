...
class TestDiscoverDataHierarchy:
    ...
    def test_single_file_without_refs(self, temp_dir):
        ...
        file.write_text('{"@id": "urn:uuid:standalone", "@type": "Thing"}')

        top_level, iri_map, _ = discover_data_hierarchy([file])

        assert len(top_level) == 1
        assert top_level[0] == file
        assert "urn:uuid:standalone" in iri_map

    def test_file_with_fixture_reference(self, temp_dir):
        """Non-DID credential referencing DID document - DID becomes fixture."""
        credential_file = temp_dir / "credential.json"
        credential_file.write_text(
            """{
            "@id": "urn:uuid:test-credential",
            "@type": "VerifiableCredential",
            "issuer": {"@id": "did:web:test:issuer"}
        }"""
        )
        did_file = temp_dir / "issuer-did.json"
        did_file.write_text('{"@id": "did:web:test:issuer", "@type": "DIDDocument"}')

        top_level, iri_map, _ = discover_data_hierarchy([temp_dir])

        # Credential is top-level, referenced DID document is fixture
        assert len(top_level) == 1
        assert top_level[0].name == "credential.json"
        assert "did:web:test:issuer" in iri_map
        assert iri_map["did:web:test:issuer"].name == "issuer-did.json"

    def test_did_document_always_fixture(self, temp_dir):
        """All DID documents are fixtures, even without references."""
        did_file = temp_dir / "standalone-did.json"
        did_file.write_text(
            '{"@id": "did:web:test:standalone", "@type": "DIDDocument"}'
        )

        top_level, iri_map, _ = discover_data_hierarchy([temp_dir])

        # DID document should NOT be top-level (it's a fixture)
        assert len(top_level) == 0
        assert "did:web:test:standalone" in iri_map

    def test_non_did_credentials_always_top_level(self, temp_dir):
        """Credentials referencing other credentials are all top-level."""
        main_credential = temp_dir / "main-credential.json"
        main_credential.write_text(
            """{
            "@id": "urn:uuid:main-credential",
            "@type": "VerifiableCredential",
            "evidence": {"@id": "urn:uuid:base-credential"}
        }"""
        )
        base_credential = temp_dir / "base-credential.json"
        base_credential.write_text(
            '{"@id": "urn:uuid:base-credential", "@type": "VerifiableCredential"}'
        )

        top_level, iri_map, _ = discover_data_hierarchy([temp_dir])

        # Both credentials should be top-level (not treated as fixtures)
        assert len(top_level) == 2
        assert any(f.name == "main-credential.json" for f in top_level)
        assert any(f.name == "base-credential.json" for f in top_level)

    def test_did_scheme_detection(self, temp_dir):
        """File with did: scheme @id is treated as DID document."""
        # File without -did suffix but with did: scheme
        did_file = temp_dir / "issuer.json"
        did_file.write_text('{"@id": "did:web:example.com:issuer", "@type": "Thing"}')
        credential = temp_dir / "credential.json"
        ...
        # issuer.json has did: scheme so it's a DID document fixture
        assert len(top_level) == 1
        assert top_level[0].name == "credential.json"

    def test_did_suffix_detection(self, temp_dir):
        """File with -did suffix is treated as DID document."""
        did_file = temp_dir / "participant-did.json"
        did_file.write_text(
            '{"@id": "https://example.com/participant", "@type": "Thing"}'
        )
        credential = temp_dir / "credential.json"
        credential.write_text(
            """{
            "@id": "urn:uuid:test",
            "subject": {"@id": "https://example.com/participant"}
        }"""
        )

        top_level, iri_map, _ = discover_data_hierarchy([temp_dir])

        # participant-did.json has -did suffix so it's a DID document fixture
        assert len(top_level) == 1
        assert top_level[0].name == "credential.json"
    ...
    def test_directory_recursive_scan(self, temp_dir):
        ...
        main_file.write_text('{"@id": "urn:uuid:main", "@type": "TestClass"}')
        nested_file = subdir / "nested.json"
        nested_file.write_text('{"@id": "urn:uuid:nested", "@type": "TestClass"}')
    ...
    def test_explicit_file_always_validated(self, temp_dir):
        """Explicitly specified DID document is top-level even if referenced."""
        credential_file = temp_dir / "credential.json"
        credential_file.write_text(
            """{
        ...
        top_level, iri_map, _ = discover_data_hierarchy([credential_file, did_file])
    ...
    def test_multiple_dirs(self, temp_dir):
        ...
        (dir1 / "file1.json").write_text('{"@id": "urn:uuid:file1"}')
        (dir2 / "file2.json").write_text('{"@id": "urn:uuid:file2"}')

        top_level, iri_map, _ = discover_data_hierarchy([dir1, dir2])
    ...
    def test_empty_paths(self, temp_dir):
        ...
        top_level, iri_map, _ = discover_data_hierarchy([])
        assert top_level == []
        assert iri_map == {}
    ...
    def test_nonexistent_path_ignored(self, temp_dir):
        ...
        valid_file.write_text('{"@id": "urn:uuid:valid"}')
    ...
    def test_detects_duplicate_ids(self, temp_dir):
        """Duplicate IDs across files are reported in metadata."""
        file1 = temp_dir / "credential-v1.json"
        file1.write_text('{"@id": "urn:uuid:duplicate-id", "@type": "Credential"}')
        file2 = temp_dir / "credential-v2.json"
        file2.write_text('{"@id": "urn:uuid:duplicate-id", "@type": "Credential"}')
        file3 = temp_dir / "unique.json"
        file3.write_text('{"@id": "urn:uuid:unique-id", "@type": "Credential"}')

        top_level, iri_map, metadata = discover_data_hierarchy([temp_dir])

        # All 3 files should be top-level
        assert len(top_level) == 3

        # Should detect the duplicate
        assert len(metadata["duplicate_ids"]) == 1
        dup_id, dup_files = metadata["duplicate_ids"][0]
        assert dup_id == "urn:uuid:duplicate-id"
        assert len(dup_files) == 2
        assert any(f.name == "credential-v1.json" for f in dup_files)
        assert any(f.name == "credential-v2.json" for f in dup_files)

    def test_no_duplicates_returns_empty_list(self, temp_dir):
        """No duplicate IDs returns empty duplicate list."""
        file1 = temp_dir / "file1.json"
        file1.write_text('{"@id": "urn:uuid:id1", "@type": "Thing"}')
        file2 = temp_dir / "file2.json"
        file2.write_text('{"@id": "urn:uuid:id2", "@type": "Thing"}')

        top_level, iri_map, metadata = discover_data_hierarchy([temp_dir])

        assert len(metadata["duplicate_ids"]) == 0

    def test_metadata_fixture_count(self, temp_dir):
        """Metadata includes correct fixture count."""
        # Create 2 credentials and 3 DID documents
        (temp_dir / "cred1.json").write_text('{"@id": "urn:uuid:cred1"}')
        (temp_dir / "cred2.json").write_text('{"@id": "urn:uuid:cred2"}')
        (temp_dir / "issuer-did.json").write_text('{"@id": "did:web:issuer"}')
        (temp_dir / "holder-did.json").write_text('{"@id": "did:web:holder"}')
        (temp_dir / "verifier-did.json").write_text('{"@id": "did:web:verifier"}')

        top_level, iri_map, metadata = discover_data_hierarchy([temp_dir])

        assert len(top_level) == 2  # Only credentials
        assert metadata["fixture_count"] == 3  # 3 DID documents
