...
|ContentShape|scenario|<a id="prop-https---w3id-org-ascs-ev-envited-x-scenario-v6-entitytypes"></a>entityTypes|0||Specifies the types of traffic entities present in the scenario. Union of the ASAM OpenSCENARIO XML V1.4.0 VehicleCategory, PedestrianCategory and MiscObjectCategory enumerations (40 values), as defined in submodules/asam-openx-standards/standards/asam-openscenario-xml/schema/OpenSCENARIO.xsd. V1.4.0 added ten vehicle categories over v1.3.1: aircraft, heavyTruck, landVehicle, micromobilityDevice, motorcycle, other, semitractor, standupScooter, watercraft and workMachine. One of them, 'other', is ASAM's own catch-all, so the enumeration admits an explicit unspecified-vehicle value while remaining closed to arbitrary strings. No value in any of the three enumerations is deprecated. Note motorcycle and motorbike coexist: motorcycle is the OpenSCENARIO spelling, motorbike the ASAM OpenDRIVE e_objectType one, and both are accepted so cross-domain assets stay valid.||scenario.shacl.ttl|
...
|ContentShape|scenario|<a id="prop-https---w3id-org-ascs-ev-envited-x-scenario-v6-weathersummary"></a>weatherSummary|0|1|Coarse weather/environment classification. Deliberately NOT set-equal to any ASAM enumeration: it is an ENVITED-X classification harmonised with ASAM OpenSCENARIO XML V1.4.0 (PrecipitationType: dry, rain, snow; FractionalCloudCover: zeroOktas to nineOktas) and ASAM OSI v3 EnvironmentalConditions (Precipitation, Fog, AmbientIllumination), collapsing those finer-grained vocabularies into search-friendly categories. Do not register it as an enumeration drift target - unlike scenario:entityTypes, no standard enumeration is being transcribed here.||scenario.shacl.ttl|
