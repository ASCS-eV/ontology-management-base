...
def builtin_data_root() -> Path:
    ...
    Source layout: the repository root (this file is ``src/tools/core/paths.py``,
    so the root is three parents above ``src`` — i.e. ``parents[3]``). Callers may
    still pass an explicit ``root_dir`` to override this default.
    """
